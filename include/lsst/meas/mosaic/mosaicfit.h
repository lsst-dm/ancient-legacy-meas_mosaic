...
public:
    ...
    typedef int ExpType;
    explicit Source(afw::table::SourceRecord const& record) :
    ...
    Source(afw::table::SimpleRecord const& record, afw::image::Wcs const& wcs) :
        _id(record.getId()), _chip(UNSET), _exp(UNSET), _sky(record.getRa(), record.getDec()),
        _pixels(wcs.skyToPixel(_sky)),
        _flux(record.get(record.getSchema().find<double>("flux").key)),
        _astromBad(!utils::isfinite(_flux)) {}
    Source(afw::coord::Coord coord, double flux=std::numeric_limits<double>::quiet_NaN()) :
        _id(-1), _chip(UNSET), _exp(UNSET), _sky(coord),
        _pixels(afw::geom::Point2D(std::numeric_limits<double>::quiet_NaN(),
    ...
    afw::coord::Coord getSky() const { return _sky; }
    afw::geom::Angle getRa() const { return getSky().getLongitude(); }
    afw::geom::Angle getDec() const { return getSky().getLatitude(); }
    afw::geom::Point2D getPixels() const { return _pixels; }
...
private:
    ...
    afw::coord::Coord _sky;     // Sky coordinates
    afw::geom::Point2D _pixels; // Pixel coordinates
    double _flux;                     // Flux
...
typedef std::map<int, afw::image::Wcs::Ptr> WcsDic;
...
public:
    ...
    int order;
    int ncoeff;
    Eigen::VectorXi xorder;
    Eigen::VectorXi yorder;
...
public:
    ...
    void setUV(afw::cameraGeom::Ccd::Ptr const &ccd, double x0=0, double y0=0);
...
public:
    ...
    ConstPtr search(afw::coord::Coord const& sky) const;
    ConstPtr findSource(Source const& s) const;
    void add(SourceMatch const& m);
    void add(PTR(Source) s,
             afw::geom::Angle d_lim=afw::geom::Angle(0, afw::geom::degrees));
    int count();
    ...
    double distance(Source const& s) const {
        return c.angularSeparation(afw::coord::Coord(s.getRa(), s.getDec())).asDegrees();
    }
...
public:
    ...
    FluxFitParams(daf::base::PropertySet::Ptr& metadata);
...
typedef std::vector<afw::cameraGeom::Ccd::Ptr> CcdSet;
...
afw::image::TanWcs::Ptr wcsFromCoeff(Coeff::Ptr& coeff);
