...
#ifdef USE_MKL
Eigen::VectorXd solveMatrix_MKL(Eigen::MatrixXd const & a, Eigen::VectorXd const & b) {
    // NOTE: all the strides and sizes below should be the same, but we use the Eigen methods
    // that correspond directly to what MKL wants in case that is not true in the future.
    assert(a.rows() == a.cols() && a.rows() == b.size());
    //char L = 'L';
    MKL_INT n = a.rows();
    MKL_INT nrhs = 1;
    MKL_INT lda = a.colStride();
    boost::scoped_array<MKL_INT> ipiv(new MKL_INT[a.size()]);
    MKL_INT ldb = b.colStride();
    MKL_INT info = 0;

    Eigen::VectorXd c(b);

    //dgesv(&n, &nrhs, a, &lda, ipiv, b, &ldb, &info);
    dgesv(&n, &nrhs, a.data(), &lda, ipiv.get(), c.data(), &ldb, &info);
    //dposv(&L, &n, &nrhs, a, &lda, b, &ldb, &info);

    return c;
}
#else
Eigen::VectorXd solveMatrix_Eigen(Eigen::MatrixXd const & a, Eigen::VectorXd const & b) {
    Eigen::PartialPivLU<Eigen::MatrixXd> lu(a);
    return lu.solve(b);
}
#endif

Eigen::VectorXd solveMatrix(Eigen::MatrixXd const & a, Eigen::VectorXd const & b) {
#ifdef USE_MKL
    return solveMatrix_MKL(a, b);
#else
    return solveMatrix_Eigen(a, b);
#endif
}

// FIXME: wholesale copied from afw::image::TanWcs.cc; should expose that instead.
void decodeSipHeader(CONST_PTR(daf::base::PropertySet) const& fitsMetadata,
...
Poly::Poly(int order_) :
    order(order_),
    ncoeff((order+1) * (order+2) / 2 - 1),
    xorder(ncoeff),
    yorder(ncoeff)
{
    int k = 0;
...
Coeff::Coeff(int order) :
    p(new Poly(order)),
    a(Eigen::VectorXd::Zero(p->ncoeff)),
    b(Eigen::VectorXd::Zero(p->ncoeff)),
    ap(Eigen::VectorXd::Zero(p->ncoeff)),
    bp(Eigen::VectorXd::Zero(p->ncoeff))
{}

Coeff::Coeff(Poly::Ptr const & p_) :
    p(p_),
    a(Eigen::VectorXd::Zero(p->ncoeff)),
    b(Eigen::VectorXd::Zero(p->ncoeff)),
    ap(Eigen::VectorXd::Zero(p->ncoeff)),
...
double Coeff::dxidu(double u, double v) {
    ...
    for (int i = 0; i < this->p->ncoeff; i++) {
        if (this->p->xorder[i]-1 >= 0) {
            dxi += this->a[i] * this->p->xorder[i]
                * pow(u, this->p->xorder[i]-1) * pow(v, this->p->yorder[i]);
        }
    }
    return dxi;
}
...
double Coeff::dxidv(double u, double v) {
    ...
    for (int i = 0; i < this->p->ncoeff; i++) {
        if (this->p->yorder[i]-1 >= 0) {
            dxi += this->a[i] * pow(u, this->p->xorder[i])
                * this->p->yorder[i] * pow(v, this->p->yorder[i]-1);
        }
    }
    return dxi;
}
...
double Coeff::detadv(double u, double v) {
    ...
    for (int i = 0; i < this->p->ncoeff; i++) {
        if (this->p->yorder[i]-1 >= 0) {
            deta += this->b[i] * pow(u, this->p->xorder[i])
                * this->p->yorder[i] * pow(v, this->p->yorder[i]-1);
        }
    }
    return deta;
...
void Obs::setUV(afw::cameraGeom::Ccd::Ptr const &ccd, double x0, double y0) {
    afw::geom::PointD  center = ccd->getCenter().getPixels(1.0);

    afw::cameraGeom::Orientation ori = ccd->getOrientation();
...
FluxFitParams::FluxFitParams(daf::base::PropertySet::Ptr& metadata) :
...
void KDTree::_initializeSources(SourceSet& s, int depth)
{
    ...
    if (s.size() == 1) {
        ...
        this->c = afw::coord::Coord(this->location[0], this->location[1]);
...
KDTree::ConstPtr KDTree::search(afw::coord::Coord const& sky) const {

    afw::geom::Angle ra  = sky.getLongitude();
    afw::geom::Angle dec = sky.getLatitude();
...
void KDTree::add(SourceMatch const& m) {
    ...
    afw::geom::Angle ra  = m.first->getRa();
    afw::geom::Angle dec = m.first->getDec();
...
KDTree::ConstPtr KDTree::findSource(Source const& s) const {
    ...
    afw::geom::Angle ra  = s.getRa();
    afw::geom::Angle dec = s.getDec();
    ...
    afw::coord::Coord coord(s.getRa(), s.getDec());
...
void KDTree::add(PTR(Source) s, afw::geom::Angle d_lim) {
    afw::geom::Angle ra  = s->getRa();
    afw::geom::Angle dec = s->getDec();
...
SourceGroup KDTree::mergeSource() {
    ...
    if (this->set.size() >= 2) {
        ...
        double dec = sd / sn;
        double mag = sm / sn;
        PTR(Source) source(new Source(afw::coord::Coord(afw::geom::Point2D(ra, dec),
                                                        afw::geom::degrees), mag));
...
kdtreeSource(SourceGroup const &sourceSet,
             KDTree::Ptr rootMat,
             int nchip,
             afw::geom::Angle d_lim, unsigned int nbrightest) {
...
double calXi_d(double a, double d, double A, double D) {
    return -cos(d)*sin(a-A)*(sin(D)*cos(d)-cos(D)*sin(d)*cos(a-A))
        / pow(sin(D)*sin(d)+cos(D)*cos(d)*cos(a-A),2.)
...
Eigen::VectorXd solveForCoeff(std::vector<Obs::Ptr>& objList, Poly::Ptr p) {
    ...
    for (size_t k = 0; k < objList.size(); k++) {
        ...
        if (o->good) {
            for (int j = 0; j < ncoeff; j++) {
                pu[j] = pow(o->u, p->xorder[j]);
                pv[j] = pow(o->v, p->yorder[j]);
            }
            for (int j = 0; j < ncoeff; j++) {
    ...
    return solveMatrix(a_data, b_data);
}
...
Eigen::VectorXd solveForCoeffWithOffset(std::vector<Obs::Ptr>& objList, Coeff::Ptr& c, Poly::Ptr p) {
    ...
    for (size_t i = 0; i < objList.size(); i++) {
        ...
        if (o->good) {
            ...
            double Cy = 0.0;
            for (int k = 0; k < ncoeff; k++) {
                pu[k] = pow(o->u, p->xorder[k]);
                pv[k] = pow(o->v, p->yorder[k]);
            }
            for (int k = 0; k < ncoeff; k++) {
                Ax -= a[k] * pu[k] * pv[k];
                Ay -= b[k] * pu[k] * pv[k];
                Bx += a[k] * pow(o->u, p->xorder[k]-1) * pv[k] * p->xorder[k];
                By += b[k] * pow(o->u, p->xorder[k]-1) * pv[k] * p->xorder[k];
                Cx += a[k] * pu[k] * pow(o->v, p->yorder[k]-1) * p->yorder[k];
                Cy += b[k] * pu[k] * pow(o->v, p->yorder[k]-1) * p->yorder[k];
            }
            for (int k = 0; k < ncoeff; k++) {
...
double calcChi(std::vector<Obs::Ptr>& objList, Eigen::VectorXd const & a, Poly::Ptr p) {
    int ncoeff = p->ncoeff;
    ...
    for (size_t k = 0; k < objList.size(); k++) {
        ...
        if (o->good) {
            ...
            double Ay = o->eta;
            for (int i = 0; i < ncoeff; i++) {
                Ax -= a[i]        * pow(o->u, p->xorder[i]) * pow(o->v, p->yorder[i]);
                Ay -= a[i+ncoeff] * pow(o->u, p->xorder[i]) * pow(o->v, p->yorder[i]);
            }
...
double flagObj(std::vector<Obs::Ptr>& objList, Eigen::VectorXd const & a, Poly::Ptr p, double e2) {
    int ncoeff = p->ncoeff;
    ...
    for (size_t j = 0; j < objList.size(); j++) {
        ...
        double Ay = 0.0;
        for (int i = 0; i < ncoeff; i++) {
            double pu = pow(o->u, p->xorder[i]);
            double pv = pow(o->v, p->yorder[i]);
            Ax += a[i]        * pu * pv;
            Ay += a[i+ncoeff] * pu * pv;
        }
        Ax -= (o->xi_A  * a[2*ncoeff] + o->xi_D  * a[2*ncoeff+1]);
               ...
               bool allowRotation=true)
{
    ...
    Eigen::MatrixXd a_data = Eigen::MatrixXd::Zero(size, size);
    Eigen::VectorXd b_data = Eigen::VectorXd::Zero(size, size);
    ...
    if (solveCcd) {
        for (int i = 0; i < nobs; i++) {
            ...
            double Dx = 0.0;
            double Dy = 0.0;
            Eigen::VectorXd const & a = coeffVec[o[i]->iexp]->a;
            Eigen::VectorXd const & b = coeffVec[o[i]->iexp]->b;
            for (int k = 0; k < ncoeff; k++) {
                pu[k] = pow(o[i]->u, p->xorder[k]);
                pv[k] = pow(o[i]->v, p->yorder[k]);
            }
            for (int k = 0; k < ncoeff; k++) {
                Ax -= a[k] * pu[k]   * pv[k];
                Ay -= b[k] * pu[k]   * pv[k];
                Bx += a[k] * pow(o[i]->u, p->xorder[k]-1) * pv[k]   * p->xorder[k];
                By += b[k] * pow(o[i]->u, p->xorder[k]-1) * pv[k]   * p->xorder[k];
                Cx += a[k] * pu[k]   * pow(o[i]->v, p->yorder[k]-1) * p->yorder[k];
                Cy += b[k] * pu[k]   * pow(o[i]->v, p->yorder[k]-1) * p->yorder[k];
                Dx += a[k] * pow(o[i]->u, p->xorder[k]-1) * pow(o[i]->v, p->yorder[k]-1)
                    * (-p->xorder[k]*o[i]->v*o[i]->v0 + p->yorder[k]*o[i]->u*o[i]->u0);
                Dy += b[k] * pow(o[i]->u, p->xorder[k]-1) * pow(o[i]->v, p->yorder[k]-1)
                    * (-p->xorder[k]*o[i]->v*o[i]->v0 + p->yorder[k]*o[i]->u*o[i]->u0);
            }
            ...
            for (int k = 0; k < ncoeff; k++) {
                ...
                // coeff x coeff
                for (int j = 0; j < ncoeff; j++) {
                    a_data(j+       ncoeff*2*o[i]->iexp, k+       ncoeff*2*o[i]->iexp) 
                        += pu[j] * pv[j] * pu[k] * pv[k];
                    a_data(j+ncoeff+ncoeff*2*o[i]->iexp, k+ncoeff+ncoeff*2*o[i]->iexp)
                        += pu[j] * pv[j] * pu[k] * pv[k];
                }

                // coeff x chip
                a_data(k+       ncoeff*2*o[i]->iexp, ncoeff*2*nexp+o[i]->ichip*np  ) += Bx * pu[k] * pv[k];
                a_data(k+       ncoeff*2*o[i]->iexp, ncoeff*2*nexp+o[i]->ichip*np+1) += Cx * pu[k] * pv[k];
                a_data(k+ncoeff+ncoeff*2*o[i]->iexp, ncoeff*2*nexp+o[i]->ichip*np  ) += By * pu[k] * pv[k];
                a_data(k+ncoeff+ncoeff*2*o[i]->iexp, ncoeff*2*nexp+o[i]->ichip*np+1) += Cy * pu[k] * pv[k];
                a_data(ncoeff*2*nexp+o[i]->ichip*np  , k+       ncoeff*2*o[i]->iexp) += Bx * pu[k] * pv[k];
                a_data(ncoeff*2*nexp+o[i]->ichip*np+1, k+       ncoeff*2*o[i]->iexp) += Cx * pu[k] * pv[k];
                a_data(ncoeff*2*nexp+o[i]->ichip*np  , k+ncoeff+ncoeff*2*o[i]->iexp) += By * pu[k] * pv[k];
                a_data(ncoeff*2*nexp+o[i]->ichip*np+1, k+ncoeff+ncoeff*2*o[i]->iexp) += Cy * pu[k] * pv[k];
                if (allowRotation) {
                    a_data(k+       ncoeff*2*o[i]->iexp,ncoeff*2*nexp+o[i]->ichip*np+2) += Dx * pu[k] * pv[k];
                    a_data(k+ncoeff+ncoeff*2*o[i]->iexp,ncoeff*2*nexp+o[i]->ichip*np+2) += Dy * pu[k] * pv[k];
                    a_data(ncoeff*2*nexp+o[i]->ichip*np+2,k+       ncoeff*2*o[i]->iexp) += Dx * pu[k] * pv[k];
                    a_data(ncoeff*2*nexp+o[i]->ichip*np+2,k+ncoeff+ncoeff*2*o[i]->iexp) += Dy * pu[k] * pv[k];
                }
            }

            // chip x chip
            a_data(ncoeff*2*nexp+o[i]->ichip*np  , ncoeff*2*nexp+o[i]->ichip*np  ) += Bx * Bx + By * By;
            a_data(ncoeff*2*nexp+o[i]->ichip*np  , ncoeff*2*nexp+o[i]->ichip*np+1) += Bx * Cx + By * Cy;
            a_data(ncoeff*2*nexp+o[i]->ichip*np+1, ncoeff*2*nexp+o[i]->ichip*np  ) += Cx * Bx + Cy * By;
            a_data(ncoeff*2*nexp+o[i]->ichip*np+1, ncoeff*2*nexp+o[i]->ichip*np+1) += Cx * Cx + Cy * Cy;
            if (allowRotation) {
                a_data(ncoeff*2*nexp+o[i]->ichip*np  , ncoeff*2*nexp+o[i]->ichip*np+2) += Bx * Dx + By * Dy;
                a_data(ncoeff*2*nexp+o[i]->ichip*np+1, ncoeff*2*nexp+o[i]->ichip*np+2) += Cx * Dx + Cy * Dy;
                a_data(ncoeff*2*nexp+o[i]->ichip*np+2, ncoeff*2*nexp+o[i]->ichip*np  ) += Dx * Bx + Dy * By;
                a_data(ncoeff*2*nexp+o[i]->ichip*np+2, ncoeff*2*nexp+o[i]->ichip*np+1) += Dx * Cx + Dy * Cy;
                a_data(ncoeff*2*nexp+o[i]->ichip*np+2, ncoeff*2*nexp+o[i]->ichip*np+2) += Dx * Dx + Dy * Dy;
            }
                    ...
                    bool allowRotation=true)
{
    ...
    Eigen::VectorXi num = Eigen::VectorXi::Zero(nstar);
    ...
    try {
        b_data = Eigen::VectorXd::Zero(size);
    } catch (std::bad_alloc) {
    ...
    if (solveCcd) {
        for (int i = 0; i < nobs; i++) {
            ...
            for (int k = 0; k < ncoeff; k++) {
                ...
                a_data(ncoeff*2*nexp+o[i]->ichip*np+1, k+ncoeff+ncoeff*2*o[i]->iexp) += Cy * pu[k] * pv[k];
                if (allowRotation) {
                    a_data(k+       ncoeff*2*o[i]->iexp, ncoeff*2*nexp+o[i]->ichip*np+2)
                        += Dx * pu[k] * pv[k];
                    a_data(k+ncoeff+ncoeff*2*o[i]->iexp, ncoeff*2*nexp+o[i]->ichip*np+2)
                        += Dy * pu[k] * pv[k];
                    a_data(ncoeff*2*nexp+o[i]->ichip*np+2, k+       ncoeff*2*o[i]->iexp)
                        += Dx * pu[k] * pv[k];
                    a_data(ncoeff*2*nexp+o[i]->ichip*np+2, k+ncoeff+ncoeff*2*o[i]->iexp)
                        += Dy * pu[k] * pv[k];
                }
            }
        ...
        for (int i = 0; i < nSobs; i++) {
            ...
            double Dx = 0.0;
            double Dy = 0.0;
            Eigen::VectorXd const & a = coeffVec[s[i]->iexp]->a;
            Eigen::VectorXd const & b = coeffVec[s[i]->iexp]->b;
            for (int k = 0; k < ncoeff; k++) {
                pu[k] = pow(s[i]->u, p->xorder[k]);
                pv[k] = pow(s[i]->v, p->yorder[k]);
            }
            for (int k = 0; k < ncoeff; k++) {
                Ax -= a[k] * pu[k]   * pv[k];
                Ay -= b[k] * pu[k]   * pv[k];
                Bx += a[k] * pow(s[i]->u, p->xorder[k]-1) * pv[k]   * p->xorder[k];
                By += b[k] * pow(s[i]->u, p->xorder[k]-1) * pv[k]   * p->xorder[k];
                Cx += a[k] * pu[k]   * pow(s[i]->v, p->yorder[k]-1) * p->yorder[k];
                Cy += b[k] * pu[k]   * pow(s[i]->v, p->yorder[k]-1) * p->yorder[k];
                Dx += a[k] * pow(s[i]->u, p->xorder[k]-1) * pow(s[i]->v, p->yorder[k]-1)
                    * (-p->xorder[k]*s[i]->v*s[i]->v0+p->yorder[k]*s[i]->u*s[i]->u0);
                Dy += b[k] * pow(s[i]->u, p->xorder[k]-1) * pow(s[i]->v, p->yorder[k]-1)
                    * (-p->xorder[k]*s[i]->v*s[i]->v0+p->yorder[k]*s[i]->u*s[i]->u0);
            }
            ...
            for (int k = 0; k < ncoeff; k++) {
                ...
                // coeff x coeff
                for (int j = 0; j < ncoeff; j++) {
                    a_data(j+       ncoeff*2*s[i]->iexp, k+       ncoeff*2*s[i]->iexp)
                        += pu[j] * pv[j] * pu[k] * pv[k];
                    a_data(j+ncoeff+ncoeff*2*s[i]->iexp, k+ncoeff+ncoeff*2*s[i]->iexp)
                        += pu[j] * pv[j] * pu[k] * pv[k];
                }

                // coeff x chip
                a_data(k+       ncoeff*2*s[i]->iexp, ncoeff*2*nexp+s[i]->ichip*np  ) += Bx * pu[k] * pv[k];
                a_data(k+       ncoeff*2*s[i]->iexp, ncoeff*2*nexp+s[i]->ichip*np+1) += Cx * pu[k] * pv[k];
                a_data(k+ncoeff+ncoeff*2*s[i]->iexp, ncoeff*2*nexp+s[i]->ichip*np  ) += By * pu[k] * pv[k];
                a_data(k+ncoeff+ncoeff*2*s[i]->iexp, ncoeff*2*nexp+s[i]->ichip*np+1) += Cy * pu[k] * pv[k];
                a_data(ncoeff*2*nexp+s[i]->ichip*np  , k+       ncoeff*2*s[i]->iexp) += Bx * pu[k] * pv[k];
                a_data(ncoeff*2*nexp+s[i]->ichip*np+1, k+       ncoeff*2*s[i]->iexp) += Cx * pu[k] * pv[k];
                a_data(ncoeff*2*nexp+s[i]->ichip*np  , k+ncoeff+ncoeff*2*s[i]->iexp) += By * pu[k] * pv[k];
                a_data(ncoeff*2*nexp+s[i]->ichip*np+1, k+ncoeff+ncoeff*2*s[i]->iexp) += Cy * pu[k] * pv[k];
                if (allowRotation) {
                    a_data(k+       ncoeff*2*s[i]->iexp, ncoeff*2*nexp+s[i]->ichip*np+2)
                        += Dx * pu[k] * pv[k];
                    a_data(k+ncoeff+ncoeff*2*s[i]->iexp, ncoeff*2*nexp+s[i]->ichip*np+2)
                        += Dy * pu[k] * pv[k];
                    a_data(ncoeff*2*nexp+s[i]->ichip*np+2, k+       ncoeff*2*s[i]->iexp)
                        += Dx * pu[k] * pv[k];
                    a_data(ncoeff*2*nexp+s[i]->ichip*np+2, k+ncoeff+ncoeff*2*s[i]->iexp)
                        += Dy * pu[k] * pv[k];
                }

                // coeff x star
                a_data(k+       ncoeff*2*s[i]->iexp, size0+s[i]->jstar*2  ) -= s[i]->xi_a  * pu[k] * pv[k];
                a_data(k+       ncoeff*2*s[i]->iexp, size0+s[i]->jstar*2+1) -= s[i]->xi_d  * pu[k] * pv[k];
                a_data(k+ncoeff+ncoeff*2*s[i]->iexp, size0+s[i]->jstar*2  ) -= s[i]->eta_a * pu[k] * pv[k];
                a_data(k+ncoeff+ncoeff*2*s[i]->iexp, size0+s[i]->jstar*2+1) -= s[i]->eta_d * pu[k] * pv[k];
                a_data(size0+s[i]->jstar*2  , k+       ncoeff*2*s[i]->iexp) -= s[i]->xi_a  * pu[k] * pv[k];
                a_data(size0+s[i]->jstar*2+1, k+       ncoeff*2*s[i]->iexp) -= s[i]->xi_d  * pu[k] * pv[k];
                a_data(size0+s[i]->jstar*2  , k+ncoeff+ncoeff*2*s[i]->iexp) -= s[i]->eta_a * pu[k] * pv[k];
                a_data(size0+s[i]->jstar*2+1, k+ncoeff+ncoeff*2*s[i]->iexp) -= s[i]->eta_d * pu[k] * pv[k];
            }

            // chip x chip
            a_data(ncoeff*2*nexp+s[i]->ichip*np  , ncoeff*2*nexp+s[i]->ichip*np  ) += Bx * Bx + By * By;
            a_data(ncoeff*2*nexp+s[i]->ichip*np  , ncoeff*2*nexp+s[i]->ichip*np+1) += Bx * Cx + By * Cy;
            a_data(ncoeff*2*nexp+s[i]->ichip*np+1, ncoeff*2*nexp+s[i]->ichip*np  ) += Cx * Bx + Cy * By;
            a_data(ncoeff*2*nexp+s[i]->ichip*np+1, ncoeff*2*nexp+s[i]->ichip*np+1) += Cx * Cx + Cy * Cy;
            if (allowRotation) {
                a_data(ncoeff*2*nexp+s[i]->ichip*np  , ncoeff*2*nexp+s[i]->ichip*np+2) += Bx * Dx + By * Dy;
                a_data(ncoeff*2*nexp+s[i]->ichip*np+1, ncoeff*2*nexp+s[i]->ichip*np+2) += Cx * Dx + Cy * Dy;
                a_data(ncoeff*2*nexp+s[i]->ichip*np+2, ncoeff*2*nexp+s[i]->ichip*np  ) += Dx * Bx + Dy * By;
                a_data(ncoeff*2*nexp+s[i]->ichip*np+2, ncoeff*2*nexp+s[i]->ichip*np+1) += Dx * Cx + Dy * Cy;
                a_data(ncoeff*2*nexp+s[i]->ichip*np+2, ncoeff*2*nexp+s[i]->ichip*np+2) += Dx * Dx + Dy * Dy;
            }

            // chip x star
            a_data(ncoeff*2*nexp+s[i]->ichip*np  , size0+s[i]->jstar*2  ) -= Bx*s[i]->xi_a + By*s[i]->eta_a;
            a_data(ncoeff*2*nexp+s[i]->ichip*np  , size0+s[i]->jstar*2+1) -= Bx*s[i]->xi_d + By*s[i]->eta_d;
            a_data(ncoeff*2*nexp+s[i]->ichip*np+1, size0+s[i]->jstar*2  ) -= Cx*s[i]->xi_a + Cy*s[i]->eta_a;
            a_data(ncoeff*2*nexp+s[i]->ichip*np+1, size0+s[i]->jstar*2+1) -= Cx*s[i]->xi_d + Cy*s[i]->eta_d;
            a_data(size0+s[i]->jstar*2  , ncoeff*2*nexp+s[i]->ichip*np  ) -= Bx*s[i]->xi_a + By*s[i]->eta_a;
            a_data(size0+s[i]->jstar*2+1, ncoeff*2*nexp+s[i]->ichip*np  ) -= Bx*s[i]->xi_d + By*s[i]->eta_d;
            a_data(size0+s[i]->jstar*2  , ncoeff*2*nexp+s[i]->ichip*np+1) -= Cx*s[i]->xi_a + Cy*s[i]->eta_a;
            a_data(size0+s[i]->jstar*2+1, ncoeff*2*nexp+s[i]->ichip*np+1) -= Cx*s[i]->xi_d + Cy*s[i]->eta_d;
            if (allowRotation) {
                a_data(ncoeff*2*nexp+s[i]->ichip*np+2,size0+s[i]->jstar*2  )-= Dx*s[i]->xi_a + Dy*s[i]->eta_a;
                a_data(ncoeff*2*nexp+s[i]->ichip*np+2,size0+s[i]->jstar*2+1)-= Dx*s[i]->xi_d + Dy*s[i]->eta_d;
                a_data(size0+s[i]->jstar*2  ,ncoeff*2*nexp+s[i]->ichip*np+2)-= Dx*s[i]->xi_a + Dy*s[i]->eta_a;
                a_data(size0+s[i]->jstar*2+1,ncoeff*2*nexp+s[i]->ichip*np+2)-= Dx*s[i]->xi_d + Dy*s[i]->eta_d;
            }

            // star x star
            a_data(size0+s[i]->jstar*2  , size0+s[i]->jstar*2  )
                += s[i]->xi_a*s[i]->xi_a + s[i]->eta_a*s[i]->eta_a;
            a_data(size0+s[i]->jstar*2  , size0+s[i]->jstar*2+1)
                += s[i]->xi_a*s[i]->xi_d + s[i]->eta_a*s[i]->eta_d;
            a_data(size0+s[i]->jstar*2+1, size0+s[i]->jstar*2  )
                += s[i]->xi_d*s[i]->xi_a + s[i]->eta_d*s[i]->eta_a;
            a_data(size0+s[i]->jstar*2+1, size0+s[i]->jstar*2+1)
                += s[i]->xi_d*s[i]->xi_d + s[i]->eta_d*s[i]->eta_d;
                    ...
                    FluxFitParams::Ptr p)
{
    ...
    Eigen::VectorXi num = Eigen::VectorXi::Zero(nmatch+nsource);
    ...
    for (int i = 0; i < nMobs; i++) {
        ...
        a_data(m[i]->iexp, m[i]->iexp) -= 1;
        a_data(nexp+m[i]->ichip, m[i]->iexp) -= 1;
        for (int k = 0; k < ncoeff; k++) {
            a_data(nexp+nchip+k, m[i]->iexp) -= pu[k] * pv[k];
        }
        a_data(nexp+nchip+ncoeff+m[i]->jstar, m[i]->iexp) += 1;

        a_data(m[i]->iexp, nexp+m[i]->ichip) -= 1;
        a_data(nexp+m[i]->ichip, nexp+m[i]->ichip) -= 1;
        for (int k = 0; k < ncoeff; k++) {
            a_data(nexp+nchip+k, nexp+m[i]->ichip) -= pu[k] * pv[k];
        }
        a_data(nexp+nchip+ncoeff+m[i]->jstar, nexp+m[i]->ichip) += 1;

        for (int j = 0; j < ncoeff; j++) {
            a_data(m[i]->iexp, nexp+nchip+j) -= pu[j] * pv[j];
            a_data(nexp+m[i]->ichip, nexp+nchip+j) -= pu[j] * pv[j];
            for (int k = 0; k < ncoeff; k++) {
                a_data(nexp+nchip+k, nexp+nchip+j) -= pu[j] * pv[j] * 
                    pu[k] * pv[k];
            }
            a_data(nexp+nchip+ncoeff+m[i]->jstar, nexp+nchip+j) += pu[j] * pv[j];
        }

        a_data(m[i]->iexp, nexp+nchip+ncoeff+m[i]->jstar) += 1;
        a_data(nexp+m[i]->ichip, nexp+nchip+ncoeff+m[i]->jstar) += 1;
        for (int k = 0; k < ncoeff; k++) {
            a_data(nexp+nchip+k, nexp+nchip+ncoeff+m[i]->jstar) += pu[k] * pv[k];
        }
        a_data(nexp+nchip+ncoeff+m[i]->jstar, nexp+nchip+ncoeff+m[i]->jstar) -= 1;
    ...
    for (int i = 0; i < nSobs; i++) {
        ...
        a_data(s[i]->iexp, s[i]->iexp) -= 1;
        a_data(nexp+s[i]->ichip, s[i]->iexp) -= 1;
        for (int k = 0; k < ncoeff; k++) {
            a_data(nexp+nchip+k, s[i]->iexp) -= pu[k] * pv[k];
        }
        a_data(nexp+nchip+ncoeff+s[i]->jstar, s[i]->iexp) += 1;

        a_data(s[i]->iexp, (nexp+s[i]->ichip)) -= 1;
        a_data(nexp+s[i]->ichip, nexp+s[i]->ichip) -= 1;
        for (int k = 0; k < ncoeff; k++) {
            a_data(nexp+nchip+k, nexp+s[i]->ichip) -= pu[k] * pv[k];
        }
        a_data(nexp+nchip+ncoeff+s[i]->jstar, nexp+s[i]->ichip) += 1;

        for (int j = 0; j < ncoeff; j++) {
            a_data(s[i]->iexp, nexp+nchip+j) -= pu[j] * pv[j];
            a_data(nexp+s[i]->ichip, nexp+nchip+j) -= pu[j] * pv[j];
            for (int k = 0; k < ncoeff; k++) {
                a_data(nexp+nchip+k, nexp+nchip+j) -= pu[j] * pv[j] * 
                    pu[k] * pv[k];
            }
            a_data(nexp+nchip+ncoeff+s[i]->jstar, nexp+nchip+j) += pu[j] * pv[j];
        }

        a_data(s[i]->iexp, nexp+nchip+ncoeff+s[i]->jstar) += 1;
        a_data(nexp+s[i]->ichip, nexp+nchip+ncoeff+s[i]->jstar) += 1;
        for (int k = 0; k < ncoeff; k++) {
            a_data(nexp+nchip+k, nexp+nchip+ncoeff+s[i]->jstar) += pu[k] * pv[k];
        }
        a_data(nexp+nchip+ncoeff+s[i]->jstar, nexp+nchip+ncoeff+s[i]->jstar) -= 1;
    ...
    Eigen::VectorXd solution = solveMatrix(a_data, b_data);
                            ...
                            FluxFitParams::Ptr p)
{
    ...
    Eigen::VectorXi num = Eigen::VectorXi::Zero(nsource);
    ...
    for (int i = 0; i < nMobs; i++) {
        ...
        if (p->chebyshev) {
            for (int k = 0; k < ncoeff; k++) {
                pu[k] = Tn(p->xorder[k], m[i]->u/u_max);
                pv[k] = Tn(p->yorder[k], m[i]->v/v_max);
            }
        } else {
            for (int k = 0; k < ncoeff; k++) {
                pu[k] = pow(m[i]->u/u_max, p->xorder[k]);
                pv[k] = pow(m[i]->v/v_max, p->yorder[k]);
            }
        }
        ...
        for (int j = 0; j < ncoeff; j++) {
            ...
            a_data(nexp+m[i]->ichip, nexp+nchip+j) -= pu[j] * pv[j];
            for (int k = 0; k < ncoeff; k++) {
                a_data(nexp+nchip+k, nexp+nchip+j) -= pu[j] * pv[j] * pu[k] * pv[k];
            }
        }
    ...
    for (int i = 0; i < nSobs; i++) {
        ...
        if (p->chebyshev) {
            for (int k = 0; k < ncoeff; k++) {
                pu[k] = Tn(p->xorder[k], s[i]->u/u_max);
                pv[k] = Tn(p->yorder[k], s[i]->v/v_max);
            }
        } else {
            for (int k = 0; k < ncoeff; k++) {
                pu[k] = pow(s[i]->u/u_max, p->xorder[k]);
                pv[k] = pow(s[i]->v/v_max, p->yorder[k]);
            }
        }
...
double calcChi2(std::vector<Obs::Ptr>& o, Coeff::Ptr c, Poly::Ptr p)
{
    ...
    for (int i = 0; i < nobs; i++) {
        ...
        double Ay = o[i]->eta;
        for (int k = 0; k < ncoeff; k++) {
            Ax -= c->a[k] * pow(o[i]->u, p->xorder[k])   * pow(o[i]->v, p->yorder[k]);
            Ay -= c->b[k] * pow(o[i]->u, p->xorder[k])   * pow(o[i]->v, p->yorder[k]);
        }
        chi2 += Ax * Ax + Ay * Ay;
    }
...
double calcChi2(std::vector<Obs::Ptr>& o, CoeffSet& coeffVec, Poly::Ptr p, bool norm=false)
{
    ...
    for (int i = 0; i < nobs; i++) {
        ...
        double Ay = o[i]->eta;
        for (int k = 0; k < ncoeff; k++) {
            Ax -= a[k] * pow(o[i]->u, p->xorder[k])   * pow(o[i]->v, p->yorder[k]);
            Ay -= b[k] * pow(o[i]->u, p->xorder[k])   * pow(o[i]->v, p->yorder[k]);
        }
        chi2 += Ax * Ax + Ay * Ay;
        num++;
    }
                      ...
                      CcdSet &ccdSet)
{
    ...
    for (size_t i = 0; i < all.size(); i++) {
        ...
        for (size_t j = 1; j < ss.size(); j++) {
            ...
            o->mag_cat = mag_cat;
            o->mag0 = mag_cat;
            afw::geom::PointD crval
                = wcsDic[iexp]->getSkyOrigin()->getPosition(afw::geom::radians);
...
void fluxFitRelative(ObsVec& matchVec,
    ...
    Eigen::VectorXd fsol = fluxFit_rel(matchVec, nmatch, sourceVec, nsource, nexp, nchip, ffp);
...
void fluxFitAbsolute(ObsVec& matchVec,
    ...
    Eigen::VectorXd fsol = fluxFit_abs(matchVec, nmatch, sourceVec, nsource, nexp, nchip, ffp);
...
Eigen::VectorXd solveSIP_P(Poly::Ptr p, std::vector<Obs::Ptr> &obsVec) {
    ...
    Eigen::VectorXd coeff(2*coeff);
    coeff.head(ncoeff) = solveMatrix(a_data, b_data);
    coeff.tail(ncoeff) = solveMatrix(d_data, c_data);
...
initialFit(int nexp,
    ...
    for (int i = 0; i < nexp; i++) {
        ...
        afw::geom::PointD crval
            = wcsDic[i]->getSkyOrigin()->getPosition(afw::geom::radians);
                     ...
                     bool verbose)
{
    ...
    for (int k = 0; k < 3; k++) {
        ...
        if (allowRotation) {
            for (int i = 0; i < nchip; i++) {
                afw::geom::PointD center = ccdSet[i]->getCenter().getPixels(1.0);
                afw::geom::PointD offset =
                    afw::geom::Point2D(center[0]+coeff[2*ncoeff*nexp+3*i],
                                       center[1]+coeff[2*ncoeff*nexp+3*i+1]);
                ccdSet[i]->setCenter(afw::cameraGeom::FpPoint(offset));
                afw::cameraGeom::Orientation o = ccdSet[i]->getOrientation();
            ...
            for (int i = 0; i < nchip; i++) {
                afw::geom::PointD center = ccdSet[i]->getCenter().getPixels(1.0);
                afw::geom::PointD offset =
                    afw::geom::Point2D(center[0]+coeff[2*ncoeff*nexp+2*i],
...
Coeff::Ptr
convertCoeff(Coeff::Ptr& coeff, afw::cameraGeom::Ccd::Ptr& ccd)
{
    ...
    for (int k = 0; k < p->ncoeff; k++) {
        for (int n = 0; n <= p->xorder[k]; n++) {
            for (int m = 0; m <= p->yorder[k]; m++) {
                int i = n + m;
                int j = p->xorder[k] + p->yorder[k] - n - m;
                int l = p->getIndex(i, j);
                double C =  binomial(p->xorder[k], n) *
		            binomial(p->yorder[k], m) *
		            pow(cosYaw, n) * pow(-sinYaw, p->xorder[k]-n) *
		            pow(sinYaw, m) * pow( cosYaw, p->yorder[k]-m);
    ...
    afw::geom::PointD off = ccd->getCenter().getPixels(1.0);
    ...
    for (int k = 0; k < p->ncoeff; k++) {
        for (int n = 0; n <= p->xorder[k]; n++) {
            for (int m = 0; m <= p->yorder[k]; m++) {
                ...
                double C =  binomial(p->xorder[k], n) *
		            binomial(p->yorder[k], m) *
		            pow(a, n) * pow(b, p->xorder[k]-n) *
		            pow(c, m) * pow(d, p->yorder[k]-m);
                ap[l] += coeff->ap[k] * C;
                bp[l] += coeff->bp[k] * C;
            }
        }
...
FluxFitParams::Ptr
convertFluxFitParams(Coeff::Ptr& coeff, afw::cameraGeom::Ccd::Ptr& ccd, FluxFitParams::Ptr& ffp)
{
    ...
    for (int k = 0; k < ffp->ncoeff; k++) {
        for (int n = 0; n <= ffp->xorder[k]; n++) {
            for (int m = 0; m <= ffp->yorder[k]; m++) {
                int i = n + m;
                int j = ffp->xorder[k] + ffp->yorder[k] - n - m;
                int l = newP->getIndex(i, j);
                double C =  binomial(ffp->xorder[k], n) *
		            binomial(ffp->yorder[k], m) *
		            pow(cosYaw, n) * pow(-sinYaw, ffp->xorder[k]-n) *
		            pow(sinYaw, m) * pow( cosYaw, ffp->yorder[k]-m)
		            / pow(ffp->u_max, ffp->xorder[k])
		            / pow(ffp->v_max, ffp->yorder[k]);
...
afw::image::TanWcs::Ptr
wcsFromCoeff(Coeff::Ptr& coeff)
{
    int order = coeff->p->order;

    afw::geom::PointD crval
        = afw::geom::Point2D(coeff->A*R2D, coeff->D*R2D);
    afw::geom::PointD crpix = afw::geom::Point2D(-coeff->x0, -coeff->y0);
    ...
    afw::image::TanWcs::Ptr wcs = afw::image::TanWcs::Ptr(new afw::image::TanWcs(crval, crpix, cd, sipA, sipB, sipAp, sipBp));

    return wcs;
}


//hsc::coeffFromTanWcs(afw::image::TanWcs::Ptr& tanwcs)
Coeff::Ptr coeffFromTanWcs(afw::image::Wcs::Ptr& wcs)
{

    //    daf::base::PropertyList::Ptr fitsMetadata = tanwcs->getFitsMetadata();
    daf::base::PropertyList::Ptr fitsMetadata = wcs->getFitsMetadata();
...
afw::image::Image<float>::Ptr
getJImg(Coeff::Ptr& coeff,
        afw::cameraGeom::Ccd::Ptr& ccd)
{
    ...
    afw::image::Image<float>::Ptr img(new afw::image::Image<float>(width, height));
    ...
    for (int y = 0; y != height; y++) {
        ...
        afw::image::Image<float>::x_iterator begin = img->row_begin(y);
        afw::image::Image<float>::x_iterator end   = img->row_end(y);

        for (afw::image::Image<float>::x_iterator ptr = begin; ptr != end; ptr++) {
        ...
        int width, int height)
{
    ...
    for (int y = 0; y != height; y++) {
        ...
        for (int x = 0; x < width + interpLength; x+= interpLength) {
            ...
            double u = x;
            double v = y;
            double val0 = wcs->pixArea(afw::geom::Point2D(u, v)) * deg2pix * deg2pix;
            u = xend;
            v = y;
            double val1 = wcs->pixArea(afw::geom::Point2D(u, v)) * deg2pix * deg2pix;
